...
export async function getParticipantByName(name: string) {
  const res = await fetch(`${endpoint}/participants/search?name=${name}`);
  return res.json() as Promise<Participant[]>;
}

export async function addParticipant(
  participant: Omit<Participant, "id" | "disciplines" | "results">,
...
export const removeDisciplineFromParticipant = async (
